...
export default function ChatWindow({ location, setIsOnline }) {
  ...
  const sendMessage = async (text) => {
    ...
    setHistory(prev => [msg, ...prev.filter(h => h !== msg)].slice(0, 5))
    setMessages(prev => [...prev, { sender: 'user', text: msg, data: null }])
    setInput('')
    setLoading(true)
    ...
    try {
      const res  = await fetch(`${API}/chat`, {
        ...
        body:    JSON.stringify({ message: msg, location }),
      })
    ...
    <div className="chat-root">
      ...
      <div className="chat-header">
        <div>
          ...
          <p className="chat-subtitle">Powered by GPT-4o mini · Location: {location}</p>
        </div>
        ...
        {messages.map((m, i) => (
          <div key={i} className={`chat-msg-row ${m.sender}`}>
            ...
            <div className={`chat-bubble ${m.sender}`}>
              {/* STRUCTURED LEGAL CARD */}
              {m.data ? (
                <div className="legal-card">
                  <div className="legal-card-title">
                    <span className="legal-offence-badge">{m.data.severity || 'Violation'}</span>
                    {m.data.offence}
                  </div>
                  ...
                  <div className="legal-tags-row">
                    {m.data.source && (
                      <div className="legal-tag blue">
                        📚 {typeof m.data.source === 'object' ? m.data.source.name : m.data.source}
                      </div>
                    )}
                    {m.data.safety_tip && (
                      <div className="legal-tag green">✅ {m.data.safety_tip}</div>
                    )}
                  </div>
                </div>
              ) : (
      ...
      {history.length > 0 && (
        <div className="chat-history">
          ...
          {history.map((h, i) => (
            <button key={i} className="chat-chip ghost" onClick={() => sendMessage(h)}>
              🕘 {h}
            </button>
          ))}
        </div>
